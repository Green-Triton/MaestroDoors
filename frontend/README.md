...
**Базовый путь подставляется ровно в одном месте.** Сайт публикуется в
подкаталог домена, а в датасете лежат относительные пути (`doors/...`).
Префикс добавляется в `entities/door/api/catalogApi.ts`, когда датасет
загружается. Дублировать это в селекторах или компонентах нельзя — получится
...
Форма шлёт `POST /api/leads`. Адрес бэкенда задаётся переменной `VITE_API_URL`
(шаблон — в [`.env.example`](.env.example)):

- **в разработке** оставьте пустым: запрос уходит на `/api` того же адреса,
  а Vite проксирует его на `http://localhost:3001` (см. `vite.config.ts`);
- **в продакшене** укажите адрес развёрнутого сервера — статика и бэкенд
  живут на разных доменах:

  ```bash
  VITE_API_URL=https://адрес-бэкенда npm run build
  ```
